...
captains_url = 'https://raw.githubusercontent.com/Shamsvi/CMSE-830/main/mens%20cwc/all_captains.csv'
...
# App Title
st.title('Cricket World Cup Analysis: 2007-2024')
...
if section == "None":
    ...
    team_country_mapping = {
        ...
        'Hong Kong': 'Hong Kong',
        'P.N.G': 'Papua New Guinea',  # Ensure correct mapping for P.N.G
        'Canada': 'Canada',
        'Uganda': 'Uganda'
    }
...
if section == "None":
    ...
    #Bar
    fig_bar_wins = px.bar(win_counts, 
                        x='Team', 
                        y='Wins', 
    ...
    all_matches_data_df['Country'] = all_matches_data_df['Winner'].map(team_mapping)
    country_wins = all_matches_data_df['Country'].value_counts().reset_index()
    country_wins.columns = ['Country', 'Wins']  
    fig_geo_wins = px.choropleth(country_wins,
                            locations='Country', 
                            locationmode='country names',  
                            color='Wins',  
                            hover_name='Country',  
    ...
    # Create a column for the losing team based on the winner
    ...
    # Extract numeric margin
    wc_final_data_df['Margin Numeric'] = wc_final_data_df['Margin'].apply(extract_numeric)

    # Prepare data for the scatter plot
    plot_data = wc_final_data_df[['Winner', 'Losing Team', 'Margin Numeric']].dropna()

    # Create scatter plot with hover data for winner, margin, and losing team
    fig_win_margin = px.scatter(
        ...
        hover_data={
            'Winner': True,
            'Margin Numeric': True,
            'Losing Team': True  # Include losing team in hover data
        }
    )
    with col1:
        st.plotly_chart(fig_win_margin, key = 'scatter_plot')
    with col2:
        st.dataframe(plot_data, key ='dataframe')
    
    wc_final_data_df['Match Date'] = pd.to_datetime(wc_final_data_df[['Match Year', 'Match Month', 'Match Day']].astype(str).agg('-'.join, axis=1), errors='coerce')
    ...
    fig = px.bar(winner_titles, 
                y='Winner', 
                x='Titles', 
                orientation='h',  # Horizontal bar chart
                text='Titles',  # Show the number of titles as text on the bars
                hover_data={'Final Dates': True},  # Show the final match dates on hover
...
if section == "None":
    ...
    # Step 5: Customize the layout of the plot
...
if section == "None":
    ...
    wc_final_data_df = pd.read_csv('wc_final_dataset.csv')      
    ...
    # Search bar for inputting a year
    ...
    if not filtered_data.empty:
        ...
        # Determine batting and bowling rankings for the team based on whether they are Team1 or Team2
        filtered_data['Batting Avg'] = np.where(
            filtered_data['Team'] == filtered_data['Team1'], 
            filtered_data['Team1 Avg Batting Ranking'], 
            filtered_data['Team2 Avg Batting Ranking']
        )
        
        filtered_data['Bowling Avg'] = np.where(
            filtered_data['Team'] == filtered_data['Team1'], 
            filtered_data['Team1 Avg Bowling Ranking'], 
            filtered_data['Team2 Avg Bowling Ranking']
        )

        # Extract numeric values from the Margin column using regex and convert to numeric
        filtered_data['Margin'] = filtered_data['Margin'].str.extract('(\d+)').astype(float)

        # Replace NaN values in Margin without using inplace method
        filtered_data['Margin'] = filtered_data['Margin'].fillna(0)

        # Calculate the number of matches the searched team played at each ground
        match_count_by_ground = filtered_data.groupby('Ground').size().reset_index(name=f"Number of Matches Played by {team_name}")

        # Merge the match count data into the filtered_data DataFrame
        filtered_data = pd.merge(filtered_data, match_count_by_ground, on='Ground', how='left')

        # Add the captain column by merging with captains_df based on team and year
        filtered_data = pd.merge(
            filtered_data,
            captains_df[['Team', 'Player Name', 'Year']],  # Only keep relevant columns
            left_on=['Team', 'Year'],
            right_on=['Team', 'Year'],
            how='left'
        )
        
        # Rename 'Player Name' to 'Captain'
        filtered_data.rename(columns={'Player Name': 'Captain'}, inplace=True)

        # Create scatter plot with bubble size and ground as color (without encoding)
        fig_scatter = px.scatter(
            filtered_data,
            x='Batting Avg',  # Using the correct column for batting average
            y='Bowling Avg',  # Using the correct column for bowling average
            size='Margin',  # Bubble size represents match margin
            color='Ground',  # Ground name directly without encoding
            hover_name='Team',  # Team names on hover
            hover_data={
                'Year': True,  # Include the Year in the hover
                'Winner': True,  # Include the winner in the hover
                'Against': True,  # Include the opposing team (Against) in the hover
                'Captain': True,  # Include the captain in the hover
                f"Number of Matches Played by {team_name}": True,  # Dynamic hover text based on the searched team
            },
            title=f'Team Performance (Batting vs Bowling Average by Ground) for {team_name}',
            labels={'Batting Avg': 'Batting Average', 'Bowling Avg': 'Bowling Average', 'Margin': 'Match Margin', 'Ground': 'Ground'}
        )

        # Display scatter plot
        st.dataframe(filtered_data[['Year', 'Team', 'Against', 'Winner', 'Ground', 'Captain']])
        ...
        if player_name:
            # Filter the player DataFrame based on the player's name
            player_data = player_df[player_df['Player Name'].str.contains(player_name, case=False, na=False)]

            if not player_data.empty:
                # Get the unique teams the player was part of
                teams = player_data['Team'].unique()

                # Iterate over the teams and display information
                for team in teams:
                    # Filter the data for the current team
                    team_data = player_data[player_data['Team'] == team]

                    # Calculate the total number of years and the range of years
                    total_years = len(team_data['Year'].unique())
                    year_range = f"{team_data['Year'].min()} - {team_data['Year'].max()}"

                    # Display the player's team info
                    ...
                    if not captain_data.empty:
                        # If they were a captain, show the years they were a captain
